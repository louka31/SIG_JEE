...
icon-settings"></i><span data-i18n="" class="menu-title">Configuration</span></a>
    ...
    <nav
            ...
            <form role="search" class="navbar-form navbar-right mt-1" action="cordonnees.jsp" >
...
<div class="row">
  <div class="col-12">
    <div class="content-header">Liste Des Cordonnees</div>
    <p class="content-sub-header">ce tableau contient liste Cordonnees autorises</p>
  </div>
</div>
<section id="extended">
  <div class="row">
    <div class="col-sm-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Liste Des Cordonnees <a style="margin-left:70%;"href="addcordonnees.jsp"><button type="button" class="btn btn-raised btn-success btn-min-width mr-1 mb-1"><i class="fa fa-check"></i>
                   Nouveaux Cordonnees</button></a></h4>
        </div>
        <div class="card-content">
          <div class="card-body table-responsive">
            <table class="table text-center">
              ...
              <tbody>
			   <% CordonneesService ps=new CordonneesService();
				if(request.getParameter("o")!=null){

			   List<Cordonnees> Cordonnees=ps.allCordonneess();
              int i=1;
               for(Cordonnees p:Cordonnees){
                if((p.getX_deg().contains(request.getParameter("o")))||(p.getY_deg().contains(request.getParameter("o"))))
															   {
															   %>
                <tr>
                   <td><%=p.getId()%>   </td>
                  <td>
                    <div class="custom-control custom-checkbox m-0">
                      ...
                      <label class="custom-control-label" for="item1"></label>
                                            ...
                                            <td><%=p.getY_deg()%></td>
                  <td>
                   <a href="plan.jsp" class="info p-0" data-original-title="" title="">
                      <i class="ft-eye font-medium-3 mr-2"></i>
                    </a>
                    <a href="modcordonnees.jsp?id=<%=p.getId()%>" class="success p-0" data-original-title="" title="">
...
<!--Shopping cart starts-->
    ...
    <!-- ////////////////////////////////////////////////////////////////////////////-->
<% CordonneesService psp=new CordonneesService();
			   
               List<Cordonnees> Cordonneesi=ps.getListOfCordonneess(String.valueOf(user.getId()));
              int j=1;
               for(Cordonnees p:Cordonneesi){%>
 <div class="modal fade text-left" id="danger<%=p.getId() %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel10"
                    aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header bg-danger white">
                          <h4 class="modal-title" id="myModalLabel10">Supprimer Cordonnees</h4>
                        ...
                        <div class="modal-body">
                          ...
                          <p>voulez-vouz vriment supprimer cet Cordonnees ?</p>
                          
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn grey btn-outline-secondary" data-dismiss="modal">Fermer</button>
                          <a href="DeleteCordonnees?id=<%=p.getId() %>"><button type="button" class="btn btn-outline-danger">Confirmer</button></a>
                        </div>
                      </div>
                    </div>
                  </div>
