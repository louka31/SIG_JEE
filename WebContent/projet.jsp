...
icon-settings"></i><span data-i18n="" class="menu-title">Configuration</span></a>
    ...
    <nav
            ...
            <form role="search" class="navbar-form navbar-right mt-1" action="projet.jsp" >
...
<div class="row">
  <div class="col-12">
    <div class="content-header">Liste Des Projets</div>
    <p class="content-sub-header">ce tableau contient liste projets autorises</p>
  </div>
</div>
<section id="extended">
  <div class="row">
    <div class="col-sm-12">
      <div class="card">
        <div class="card-header">
          <h4 class="card-title">Liste Des Projets <a style="margin-left:70%;"href="addprojet.jsp"><button type="button" class="btn btn-raised btn-success btn-min-width mr-1 mb-1"><i class="fa fa-check"></i>
                   Nouveau Projet</button></a></h4>
        </div>
        <div class="card-content">
          <div class="card-body table-responsive">
            <table class="table text-center">
			 ...
			 int  i=1;
          CompagneService fs= new CompagneService () ;

				if(request.getParameter("o")!=null){

               List<Projet> projet=ps.allProjets();
               for(Projet p:projet){
                   if((p.getXmax().contains(request.getParameter("o")))||((fs.getCompagnebyCompagneId(p.getIdCompagne()+"").getNom()).contains(request.getParameter("o")))||(p.getNom().contains(request.getParameter("o")))||(p.getXmin().contains(request.getParameter("o")))||(p.getYmax().contains(request.getParameter("o")))||(p.getYmin().contains(request.getParameter("o"))))
                ...
                <tr>
                   <td><%=i%>   </td>
                      ...
                      <td><%=p.getNom()%></td>
                                            ...
                                            <td><%=p.getYmin()%></td>
                                            <td><%=p.getYmin()%></td>
														<td><%= fs.getCompagnebyCompagneId(p.getIdCompagne()+"").getNom() %></td>
                  <td>
                    <a href="limitesprojet.jsp?id=<%=p.getNom()%>" class="info p-0" data-original-title="" title="">
                      <i class="ft-eye font-medium-3 mr-2"></i>
                    </a>
                    <a href="modprojet.jsp?id=<%=p.getId()%>" class="success p-0" data-original-title="" title="">
...
<!--Shopping cart starts-->
    ...
    <!-- ////////////////////////////////////////////////////////////////////////////-->
<% ProjetService psp=new ProjetService();
			   
               List<Projet> projeti=ps.getListOfProjets(String.valueOf(user.getId()));
              int j=1;
               for(Projet p:projeti){%>
 <div class="modal fade text-left" id="danger<%=p.getId() %>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel10"
                    aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header bg-danger white">
                          <h4 class="modal-title" id="myModalLabel10">Supprimer Projet</h4>
                        ...
                        <div class="modal-body">
                          ...
                          <p>voulez-vouz vriment supprimer cet projet ?</p>
                          
                        </div>
                        <div class="modal-footer">
                          <button type="button" class="btn grey btn-outline-secondary" data-dismiss="modal">Fermer</button>
                          <a href="DeleteProjet?id=<%=p.getId() %>"><button type="button" class="btn btn-outline-danger">Confirmer</button></a>
                        </div>
                      </div>
                    </div>
                  </div>
